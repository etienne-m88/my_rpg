    ...
    sfVector2f scale, sprites_t *animation)
{
    ...
    animation->bounds = (sfIntRect){0, 0, size.y - 5, size.y};
...
mob_t *new_b2mob(sfVector2f pos)
{
    mob_t* new_node = malloc(sizeof(mob_t));

    new_node->mob = malloc(sizeof(sprites_t));
    animation_mob("assets/mobs/b2.png", 0.15, (V2F){1.5, 1.5}, new_node->mob);
