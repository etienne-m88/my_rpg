    ...
    sfVector2f scale, sprites_t *animation)
{
    ...
    animation->bounds = (sfIntRect){0, 0, size.y - 6, size.y};
    animation->frame_duration = duration;
    animation->seconds = 0;
    animation->end_frame = size.x;
    animation->frame_size = size.y * 2 + 20;
...
mob_t *new_tearmob(sfVector2f pos)
{
    ...
    animation_mob("assets/mobs/tears.png", 0.15, (V2F){1.5, 1.5}, new_node->mob);
    sprite_position(pos, new_node->mob);
    new_node->position = pos;
    new_node->hitbox = rectancle((V2F){50, 50}, (V2F){pos.x, pos.y + 10}, sfColor_fromRGBA(0, 0, 0, 0));
    new_node->hp = 3;
    new_node->speed = 140;
    new_node->damage = 1;
...
void update_tear(global_t* global, mob_t *mob, float delta_time)
{
    ...
    while (tmp != NULL) {
        ...
        sfRectangleShape_setPosition(tmp->hitbox, (V2F){tmp->position.x, tmp->position.y});
