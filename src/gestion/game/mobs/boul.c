    ...
    sfVector2f scale, sprites_t *animation)
{
    ...
    animation->bounds = (sfIntRect){0, 0, size.y -10, size.y};
    animation->frame_duration = duration;
    animation->seconds = 0;
    animation->end_frame = size.x;
    animation->frame_size = 36;
...
mob_t *new_boulmob(sfVector2f pos)
{
    mob_t* new_node = malloc(sizeof(mob_t));

    new_node->mob = malloc(sizeof(sprites_t));
    animation_mob("assets/mobs/machin.png", 0.15, (V2F){2, 2}, new_node->mob);
    sprite_position(pos, new_node->mob);
    new_node->position = pos;
    new_node->hitbox = rectancle((V2F){60, 60}, (V2F){pos.x, pos.y + 10}, sfColor_fromRGBA(0, 0, 0, 0));
    new_node->hp = 20;
    new_node->speed = 100;
    new_node->damage = 5;
