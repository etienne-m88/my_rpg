...
mob_t *new_batmob(sfVector2f pos)
{
    mob_t* new_node = malloc(sizeof(mob_t));

    new_node->mob = malloc(sizeof(sprites_t));
    animation_mob("assets/mobs/bat.png", 0.15, (V2F){1.5, 1.5}, new_node->mob);
    sprite_position(pos, new_node->mob);
    new_node->position = pos;
    new_node->hitbox = rectancle((V2F){60, 60}, (V2F){pos.x + 15, pos.y + 25}, sfColor_fromRGBA(0, 0, 0, 0));
    new_node->hp = 3;
    new_node->speed = 140;
    new_node->damage = 2;
...
void update_bat(global_t* global, mob_t *mob, float delta_time)
{
    ...
    while (tmp != NULL) {
        ...
        sfRectangleShape_setPosition(tmp->hitbox, (V2F){tmp->position.x + 15, tmp->position.y + 25});
